...
{
    /* Not yet implemented. */
    return round_real_to_int(multiply_real_and_int(load_avg,100));
}
...
//THE CODE THAT IMPLEMENTED//

/****************priotity scheduling ******************/

bool cmp_priority(const struct list_elem *first_elem ,const struct list_elem *second_elem , void *aux UNUSED)
{
...
void calculating_load_avg(void)
{
    ...
    if (thread_current () != idle_thread){
        ready_threads++; }
    load_avg = add_real_to_real(divide_real_by_int(multiply_real_and_int(load_avg, 59), 60) ,
                                divide_real_by_int(convert_to_real(ready_threads), 60));
...
void update_priority_advanced(struct thread *t, void* aux UNUSED)
{
    ...
    t->priority = convert_to_integer(subtract_int_from_real(subtract_real_from_real(convert_to_real(PRI_MAX),
                                                    divide_real_by_int(t->recent_cpu, 4)),
                                          2 * t->nice));
